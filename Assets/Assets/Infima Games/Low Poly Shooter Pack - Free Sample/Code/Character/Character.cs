...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character : CharacterBehaviour
	{
		...
		public override bool IsCrosshairVisible() => equippedWeapon != null && !aiming && !holstered;
		...
		/// </summary>
		public bool AcquirePickup(WeaponBehaviour prefab)
{
 if (prefab == null || !(inventory is Inventory concrete)) return false;
 StopAllCoroutines(); concrete.Acquire(prefab);
 reloading = inspecting = holstering = holdingButtonFire = holdingButtonAim = false;
 RefreshWeaponSetup(); SetHolstered(false);
 characterAnimator.Play("Unholster", layerHolster, 0); return true;
}
