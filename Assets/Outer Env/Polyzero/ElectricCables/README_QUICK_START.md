...
Expand `Generated Cables` in `Start` to select a specific generated cable or click its `Invert` / `Restore` button. The same normal control is available when that cable is selected directly.
